...
module Modules_Frame() {
    difference() {
        union() {
            ...
            color("blue")
            translate([depth - (wall_thickness + 1 + 2*delta + breadboard_depth), wall_thickness + 1, plate_thickness])
        ...
        // bottom magnets
        magnet_hole(6, 6, -delta);
        magnet_hole(6, width/2, -delta);
        magnet_hole(6, width - 6, -delta);
        magnet_hole(depth - 6, 6, -delta);
        magnet_hole(depth - 6, width/2, -delta);
        magnet_hole(depth - 6, width - 6, -delta);
        magnet_hole(depth/2 - 6, 6, -delta);
        magnet_hole(depth/2 - 6, width - 6, -delta);
    ...
    color("red")
    translate([slot_depth+ wall_thickness + 2*delta, width, plate_thickness]) rotate([90, 0, 0]) linear_extrude(width)
    ...
    color("cyan")
    translate([wall_thickness, (width - wall_thickness)/2, plate_thickness])
        cube([slot_depth + 2*delta, wall_thickness, height]);


    color("yellow")
    translate([slot_depth + 2*wall_thickness + 2*delta, 0, plate_thickness])
        cube([breadboard_depth + 2*delta, wall_thickness + 1, height]);

    color("yellow")
    translate([slot_depth + 2*wall_thickness + 2*delta, wall_thickness + 1 + breadboard_width + 2*delta, plate_thickness]) rotate([90, 0, 90]) linear_extrude(breadboard_depth + 2*delta + wall_thickness + 1)
        polygon([[0, 0], [wall_thickness, 0], [wall_thickness + 1, height], [0, height]]);

    color("yellow")
    translate([slot_depth + 2*wall_thickness + 2*delta, width - wall_thickness, plate_thickness]) rotate([90, 0, 90]) linear_extrude(breadboard_depth + 2*delta + wall_thickness + 1)
        polygon([[0, 0], [wall_thickness, 0], [wall_thickness, height], [-1, height]]);
}
